...
namespace admittance_controller {
  ...
  bool AdmittanceRule::calculate_admittance_rule(AdmittanceState &admittance_state, double dt) {
    ...
    // Translate the force setpoint into the toolframe
    admittance_state.force_setpoint.block<3, 1>(0, 0) =
        admittance_state.ref_trans_base_ft.rotation() * admittance_state.force_setpoint.block<3, 1>(0, 0);
    admittance_state.force_setpoint.block<3, 1>(3, 0) =
        admittance_state.ref_trans_base_ft.rotation() * admittance_state.force_setpoint.block<3, 1>(3, 0);
    ...
    // Example arrays for IRB 2400 (in rad/s and rad/s^2):
    // Verify them carefully or pull from your param server.
    // (Max velocity roughly converted from ~deg/s specs)
    // (Max acceleration is an estimate; see your robot data)
    static const std::array<double, 6> MAX_VELOCITY = {
      2.617,  // ~150 deg/s for Joint 1
      2.094,  // ~120 deg/s for Joint 2
      2.094,  // ~120 deg/s for Joint 3
      3.927,  // ~225 deg/s for Joint 4
      3.927,  // ~225 deg/s for Joint 5
      3.927   // ~225 deg/s for Joint 6
    };

    static const std::array<double, 6> MAX_ACCELERATION = {
      3.0,  // rad/s^2 estimate for Joint 1
      3.0,  // ...
      3.0,
      5.0,
      5.0,
    ...
    // Now clamp each joint:
    for (size_t i = 0; i < admittance_state.joint_acc.size(); ++i)
    {
      // 1) Clamp acceleration:
      admittance_state.joint_acc[i] = clamp(
        admittance_state.joint_acc[i], -MAX_ACCELERATION[i], MAX_ACCELERATION[i]);

      // 2) Integrate velocity:
      admittance_state.joint_vel[i] += admittance_state.joint_acc[i] * dt;

      // 3) Clamp velocity:
      admittance_state.joint_vel[i] = clamp(
        admittance_state.joint_vel[i], -MAX_VELOCITY[i], MAX_VELOCITY[i]);

      // 4) Integrate position:
      admittance_state.joint_pos[i] += admittance_state.joint_vel[i] * dt;
    }
